...
fn render_table(rows: &[Vec<String>], width: usize) {
  ...
  let cell_display_w =
    |cell: &str| -> usize { to_plain(cell).chars().map(char_display_width).sum() };
  ...
  for row in rows {
    for (c, cell) in row.iter().enumerate() {
      ...
      if !is_sep {
        col_w[c] = col_w[c].max(cell_display_w(cell));
      }
    }
  }
  ...
  if total > avail && avail > 0 {
    ...
    col_w = col_w
      .iter()
      .map(|&w| ((w as f64 * scale).floor() as usize).max(4))
      .collect();
  }
...
fn render_markdown(content: &str) {
  ...
  while i < lines.len() {
    ...
    if trimmed.starts_with("```") {
      ...
      if !lang.is_empty() {
        println!("{DIM}{FG_GRAY}  ▸ {lang}{RESET}");
      }
      i += 1;
    ...
    if let Some(t) = trimmed.strip_prefix("# ") {
      ...
      println!(
        "{BG_DARK}{FG_CYAN}{BOLD}{}{RESET}",
        center_str(&banner, width)
      );
...
fn main() {
  ...
  let file_path: PathBuf = if args.len() > 1 {
    ...
    if p.exists() {
      ...
      find_reports(&reports_dir)
        .into_iter()
        .find(|f| f.to_string_lossy().contains(arg.as_str()))
        .unwrap_or_else(|| {
          eprintln!("No report matching '{arg}'");
          std::process::exit(1);
        })
    }
  } else {
    ...
    if r.is_empty() {
      eprintln!("No reports found. Run: cargo run --bin travel -- <from> <to> <days> <budget>");
      std::process::exit(1);
    }
    r.pop().unwrap()
  };
  ...
  println!(
    "{}",
    center_str(
      &format!("{FG_CYAN}{BOLD}  🌏 OpenWorld Report Viewer  {RESET}"),
      width
    )
  );
  ...
  if all.len() > 1 {
    ...
    for r in &all {
      ...
      if r == &file_path {
        println!("  {FG_GREEN}▶ {n}{RESET}");
      } else {
        println!("  {FG_GRAY}  {n}{RESET}");
      }
    }
    println!("  {FG_GRAY}{}{RESET}", hr('─', width - 2));
