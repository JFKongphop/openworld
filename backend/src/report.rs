...
The report captures:
  ...
  - Execution proof (hashes, storage root, on-chain TX)
*/
...
pub fn generate_travel_report(
  ...
  let confirmed: Vec<&BookingResult> = bookings
    .iter()
    .filter(|b| b.status == BookingStatus::Confirmed)
    .collect();
  let failed: Vec<&BookingResult> = bookings
    .iter()
  ...
  let pct_used = if budget > 0.0 {
    (total_spent / budget * 100.0).round() as u32
  } else {
    0
  };
  let status_badge = if failed.is_empty() {
    "✅ Complete"
  } else {
    "⚠️ Partial"
  };
  ...
  md.push_str("> Autonomous travel planning and reservation — powered by Qwen AI\n\n");
  ...
  push_row(
    ...
    &format!(
      "{} → {}",
      policy.trip.departure_date, policy.trip.return_date
    ),
  );
  ...
  push_row(
    &mut md,
    "Duration",
    &format!("{} days", policy.trip.duration_days),
  );
  push_row(&mut md, "Budget", &format!("${:.2} USD", budget));
  push_row(
    &mut md,
    "**Total Spent**",
    &format!("**${:.2} USD**", total_spent),
  );
  push_row(
    &mut md,
    "Remaining",
    &format!("${:.2} USD  *({}% used)*", savings, pct_used),
  );
  ...
  let remaining_pct = if budget > 0.0 {
    savings / budget * 100.0
  } else {
    0.0
  };
  md.push_str(&format!(
  ...
  for (icon, label, amt) in categories {
    ...
    let pct = if budget > 0.0 {
      (amt_display / budget * 100.0).round() as usize
    } else {
      0
    };
  ...
  md.push_str("*Planned by **PlannerAgent** via Qwen AI*\n\n");
  ...
  if let Some(itin) = itinerary {
    ...
    if (itin.estimated_total_usd - total_spent).abs() > 1.0 {
      ...
      if diff > 0.0 {
        md.push_str(&format!(
          "> 💡 Saved USD {:.0} vs initial plan through optimised pricing.\n\n",
          diff
        ));
      } else {
        md.push_str(&format!(
          "> ⚠️ Actual cost USD {:.0} over initial plan estimate.\n\n",
          diff.abs()
        ));
      }
    }
  ...
  push_row(
    &mut md,
    "✈️ Max Stops",
    &policy.flight.max_stops.to_string(),
  );
  push_row(
    &mut md,
    "✈️ Avoid Red-Eye",
    bool_str(policy.flight.avoid_red_eye),
  );
  if !policy.flight.preferred_airlines.is_empty() {
    push_row(
      &mut md,
      "✈️ Preferred Airlines",
      &policy.flight.preferred_airlines.join(", "),
    );
  }
  // Hotel
  push_row(
    &mut md,
    "🏨 Min Rating",
    &format!("⭐ {:.1}", policy.hotel.min_rating),
  );
  push_row(
    &mut md,
    "🏨 Max per Night",
  ...
  push_row(
    &mut md,
    "🏨 Near Station",
    bool_str(policy.hotel.near_station),
  );
  // Transport
  push_row(
    &mut md,
    "🚄 Prefer Train",
    bool_str(policy.transport.prefer_train),
  );
  push_row(
    &mut md,
    "🚄 Avoid Overnight Bus",
  ...
  push_row(
    &mut md,
    "🤖 Auto Reserve",
    bool_str(policy.automation.auto_reserve),
  );
  push_row(
    &mut md,
    "🤖 Retry on Failure",
    bool_str(policy.automation.retry_on_failure),
  );
  push_row(
    &mut md,
    "🤖 Allow Replanning",
    bool_str(policy.automation.allow_replanning),
  );
  // Vault
  push_row(
    &mut md,
    "💰 Auto Payment",
    bool_str(policy.vault.auto_payment),
  );
  push_row(
    &mut md,
    "💰 Max Single TX",
  ...
  md.push_str(&format!("*AI-generated travel tips for {}*\n\n", city_name));
  ...
  md.push_str(&format!(
    "- [ ] Visa checked for {} (Thai passport holders)\n",
    city_name
  ));
  ...
  // ── Footer ──────────────────────────────────────────────────────────────────
  md.push_str(&format!(
    "*Generated by **OpenWorld** Agentic Travel System · {}*\n",
    now
  ));
